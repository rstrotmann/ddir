...
The input source can either be a file name as string or a text
connection. This can be used to read the compound data from a string.
...
A perpetrator object if only one compound in the input source, or list
of perpetrator objects.
...
- 'oral' A Boolean (i.e., `TRUE` or `FALSE`) to indicate whether the
  drug is subject to first-pass effects.

- 'mw' The molar weight in g/mol as numeric.

- 'dose' The clinically administered dose in mg as numeric.

- 'imaxss' The (total) \\C\_{max}\\ in ng/ml after administration of the
  clinical dose.
...
included in the perpetrator file. The below is an example of a valid
compound file:
