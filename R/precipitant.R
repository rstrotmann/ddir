...
#'
#' @returns A precipitant class object.
#' @export
#' @examples
#' precipitant(
#'   name = "examplinib",
...
#' Print precipitant object as markdown
#'
#' @param x The precipitant object.
#' @param ... Further arguments.
#'
#' @returns A markdown-formatted table.
#' @import dplyr
#' @exportS3Method base::print
print.precipitant <- function(x, ...) {
  out <- tibble::tribble(
  ...
  if (isTRUE(getOption("knitr.in.progress"))) {
    ...
    out |>
      ...
      knitr::kable(caption = paste("Precipitant compound parameters for", x$name)) |>
      print()
  } else {
...
#'
#' @param x A precipitant compound object.
...
#' concentrations in \eqn{\mu M} and ng/ml for a DDI precipitant compound.
#'
#' @details
#' ## Gut concentration
#'
...
key_conc_table <- function(
  ...
  rownames(temp) <- NULL
  caption <- paste0("Key precipitant concentrations for ", x$name)
