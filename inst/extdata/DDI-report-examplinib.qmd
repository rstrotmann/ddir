...
transporter_inh <- inhibition_data(
  data = tibble::tribble(
    ~object, ~ic50,     ~source,
      "Pgp",  0.41, "study 005",
     "BCRP",   1.9, "study 005",
...
```{r precipitant_properties, results="asis"}
print(perp)
```

# Key precipitant concentrations

The following precipitant concentrations were calculated as described in 
Appendix 1:
...
$R=[I]/IC_{50,u}$. For the precipitant concentrations relevant for the
individual transporters, refer to Section '[Inhibition of drug transporters]' in
Appendix 1.

```{r results="asis"}
print(transporter_inhibition_risk(perp, transporter_inh))
```
